...
class HomeRepository(context: Context) {
    ...
    fun getRecommended(compositeDisposable: CompositeDisposable):LiveData<List<RecommendedResponse.Manga>> {
        networkState.postValue(NetworkState.LOADING)

        return object : LiveData<List<RecommendedResponse.Manga>>() {
            override fun onActive() {
                super.onActive()
    ...
    compositeDisposable: CompositeDisposable
    ): LiveData<List<ExploreResponse.Manga>> {
        networkState.postValue(NetworkState.LOADING)

        var i = 0
        return object : LiveData<List<ExploreResponse.Manga>>() {
            override fun onActive() {
                super.onActive()

                compositeDisposable.add(
                    apiService.getExplore(page)
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .timeout(5, TimeUnit.SECONDS)
                        .retryWhen {
